...
def delete_payment(request, id):
    ...
    return render(request, 'admin/deletePyment.html', {'payment': payment})



@login_required
def delete_paymentok(request, id):
    payment = get_object_or_404(EmployeePayment, id=id)
...
import json
@login_required
def employee_attendance_chart(request):
    employes = Employee.objects.all()
    from_date = request.GET.get('from_date')
    to_date = request.GET.get('to_date')
    ...
    if from_date and to_date:
        from_date = parse_date(from_date)
        to_date = parse_date(to_date)
        # Filter the Attendance and EmployeePayment based on the date range
        ...
        # If no date range, get all Attendance and EmployeePayment records
   ...
   # employees = [employee for employee in employes if employee.is_working and employee.calculate_balance() != 0]
    ...
    # Handle pagination for attendance records
    show_all = request.GET.get('show_all', False)
    if not show_all:
        from django.core.paginator import Paginator
        paginator = Paginator(payments, 30)  # Show 10 attendance records per page
    ...
    return render(request, 'admin/chart2.html', context)
...
def attendance_json(request):
    ...
    attendance_records = Attendance.objects.filter(date=today).annotate(
        created_by_admin=Concat(
            F('created_by__full_name'),
            Value(' '),
        )
    ).values('employee__name', 'status', 'created_by_admin', 'pk').order_by('-pk')  # Ensure we order by primary key to get the most recent first
